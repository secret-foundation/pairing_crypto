...
import pairing_crypto.Bls12381BbsG1BlsSigG2Sha256
...
class PairingCryptoRnModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256GenerateBbsKeyPair(request: ReadableMap, promise: Promise) {
    val result = WritableNativeMap()
    val ikm = ByteArray(32)
    val keyInfo = ByteArray(32)

    var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
    var keyPair = cipherSuite.generateBbsKeyPair(ikm, keyInfo)
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256GenerateBlsKeyPair(request: ReadableMap, promise: Promise) {
    val result = WritableNativeMap()
    val ikm = ByteArray(32)
    val keyInfo = ByteArray(32)

    var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
    var keyPair = cipherSuite.generateBlsKeyPair(ikm, keyInfo)
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256BlsKeyPopGen(request: ReadableMap, promise: Promise) {
    try {
      var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
      val blsSecretKey = request.getByteArray("blsSecretKey")
      val aud = request.getByteArray("aud")
      val dst = if (request.hasKey("dst")) request.getByteArray("dst") else ByteArray(0)
      val extraInfo = if (request.hasKey("extraInfo")) request.getByteArray("extraInfo") else ByteArray(0)

      val pop = cipherSuite.blsKeyPopGen(blsSecretKey, aud, dst, extraInfo)
      promise.resolve(pop.toReadableArray())
    } catch (exception: Exception) {
      promise.reject(exception)
    }
  }
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256BlsKeyPopVerify(request: ReadableMap, promise: Promise) {
    try {
      var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
      val blsKeyPop = request.getByteArray("blsKeyPop")
      val blsPublicKey = request.getByteArray("blsPublicKey")
      val aud = request.getByteArray("aud")
      val dst = if (request.hasKey("dst")) request.getByteArray("dst") else ByteArray(0)
      val extraInfo = if (request.hasKey("extraInfo")) request.getByteArray("extraInfo") else ByteArray(0)

      val verified = cipherSuite.blsKeyPopVerify(blsKeyPop, blsPublicKey, aud, dst, extraInfo)
      promise.resolve(verified)
    } catch (exception: Exception) {
      promise.reject(exception)
    }
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256Sign(request: ReadableMap, promise: Promise) {
    try {
      var secretKey: ByteArray? = null
      var publicKey: ByteArray? = null
      var blsPublicKey: ByteArray? = null
      var header: ByteArray? = null
      var messages: Array<ByteArray>? = null
      ...
      if (request.hasKey("blsPublicKey")) {
        blsPublicKey = request.getByteArray("blsPublicKey")
      }
      if (request.hasKey("header")) {
        header = request.getByteArray("header")
      }
      ...
      var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
      var signature = cipherSuite.sign(secretKey, publicKey, blsPublicKey, header, messages)
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256Verify(request: ReadableMap, promise: Promise) {
    try {
      var publicKey: ByteArray? = null
      var blsSecretKey: ByteArray? = null
      var header: ByteArray? = null
      var signature: ByteArray? = null
      var messages: Array<ByteArray>? = null
      ...
      if (request.hasKey("blsSecretKey")) {
        blsSecretKey = request.getByteArray("blsSecretKey")
      }
      if (request.hasKey("header")) {
        header = request.getByteArray("header")
      }
      ...
      var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
      var verified = cipherSuite.verify(publicKey, blsSecretKey, header, signature, messages)
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256ProofVerify(request: ReadableMap, promise: Promise) {
    try {
      var header: ByteArray? = null
      var presentationHeader: ByteArray? = null
      var publicKey: ByteArray? = null
      ...
      var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
      var verified = cipherSuite.verifyProof(publicKey, header, presentationHeader, proof, messages ?: HashMap())
  ...
  @ReactMethod
  fun Bls12381BbsG1BlsSigG2Sha256ProofGen(request: ReadableMap, promise: Promise) {
    try {
      var header: ByteArray? = null
      var presentationHeader: ByteArray? = null
      var publicKey: ByteArray? = null
      ...
      var cipherSuite = Bls12381BbsG1BlsSigG2Sha256()
      var proof = cipherSuite.createProof(publicKey, blsSecretKey, header, presentationHeader, signature, verifySignature, disclosedIndices ?: HashSet(), messages)
